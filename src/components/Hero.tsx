...
export function Hero() {
  return <section id="home" className="section pt-24">
    <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="card relative overflow-hidden">
      ...
      <h1 className="relative mt-3 max-w-4xl text-3xl font-semibold leading-tight md:text-5xl">Engineering secure, automated and resilient infrastructure for modern enterprises.</h1>
      ...
      <div className="relative mt-6 flex flex-wrap gap-2">
        {heroPillars.map((pillar, idx) => <motion.span key={pillar} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 + idx * 0.05 }} className={`rounded-full border px-3 py-1 text-xs font-medium ${pillar.includes("Team") ? "border-token-accent tag-token" : "border-token"}`}>{pillar}</motion.span>)}
      </div>
    </motion.div>
  </section>;
}
